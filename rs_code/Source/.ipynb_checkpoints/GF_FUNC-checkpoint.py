...
def GF_Add(a,b): # alpha^c = alpha^a + alpha^b
    a = np.array(a)
    b = np.array(b)
    aa=GFE[(a+1).astype(np.int)]
    bb=GFE[(b+1).astype(np.int)]
    cc=np.add(aa,bb)%2
    cc = np.reshape(cc, (-1, 6))
    Index = []
    for i in range(cc.shape[0]):
        Index.append(GF_Index(cc[i]))
    return Index
...
#   print('input coeff:',input_coeff)
    ...
    for i in range(num_computation):
        for j in range(conv_window-2,-1,-1): 
            buff[j+1]=buff[j]
...
def Parity_RS(message_RS,generator_RS, order_alpha, codeword_size):
    sr_size=len(generator_RS)-1
    sr_content=-1*np.ones((codeword_size, sr_size))
...
def POL_DIV(Dividend,Divisor, order_alpha, m):
    Dividend_coeff=[]
...
#        print('branch output:',branch_output)
        for j in range(sr_size-1,0,-1):
            sr_content[j]=GF_Add(sr_content[j-1], branch_output[j])[0]
        sr_content[0]=GF_Add(branch_output[0], Dividend_coeff[-1])[0]
...
def POL_Add(a_coeff, b_coeff):
    ...
    for i in range(min_a_b_length):
        dum=GF_Add(a_coeff[i],b_coeff[i])[0]
...
def GF_InnerProduct(A_GF_Vector,B_GF_Vector, order_alpha):
...
def GF_Function_Value(GFE_Index,Order_GFE,GF_Function_coeff):
    ...
    Function_Value=GF_InnerProduct(A_GF_Vector,GF_Function_coeff, Order_GFE)
...
def GF_Derivative(GFVector, order_alpha):
    GFVector_Prime=[]
    A_GF_Vector=[]
